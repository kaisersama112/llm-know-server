import axios, { AxiosHeaders, type InternalAxiosRequestConfig, type AxiosRequestConfig } from 'axios'
...
instance.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    // 确保 headers 是 AxiosHeaders（避免 undefined 引起的类型报错）
    config.headers = config.headers ?? new AxiosHeaders()

    const { user } = useStore()
    const token = user.getToken()
    const language: string = user.getLanguage()

    ;(config.headers as AxiosHeaders).set('Accept-Language', language)
    if (token) {
      ;(config.headers as AxiosHeaders).set('Authorization', token.toString())
    }
...
instance.interceptors.response.use(
  ...
  (err: any) => {
    ...
    if (err.response?.status === 404) {
      ...
      const ignoreRedirectPatterns = ['/application/authentication', '/ai_template']
      const shouldIgnoreRedirect = ignoreRedirectPatterns.some((pattern) =>
...
) => Promise<Result<any> | any> = (url, data, params, loading, timeout, config) => {
  return promise(
    request({
      url: url,
      method: 'post',
